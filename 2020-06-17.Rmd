...
5.3.4 switch
另一種條件式執行常用在condition種類煩雜或要全部以「是/否」問句來切出所有互斥condition情境不容易設計時。condition改成用文字描述，即conditionExpression。
不同conditionExpression與對應body內容改成使用一對對的:
"conditionExpression文字"={body 內容} 的name=value pair

switch是用文字描述來決定你要的情境，定義你的情境要做什麼。
else if是logic，switch是character字串
```{r}
switch(實現的conditionExpression,
       ...
       {    }) #最後一個沒有情境描述文字的大括號代表剩下的沒被分到各種情境的
```
...
switch(
  conditionExpression_outcome, #這個conditionExpression_outcome只能產出"even"或"odd"，所以要先判斷我們給的數字是被歸在even或odd
  "even"={fvalue <- n/2}, # 偶數
...
2.若最後一個被執行的指令會產生print在螢幕的效果，則可以用{...}->或<-{...}將這print的值存出來。
```{r}
result <- 
  {
    x <- 2
...
3.依照上一個特性，function(...){....}的body若最後一個「被執行」的指令是會印在螢幕的值，如該值要當回傳值可以不用return(...)就可以產生值回傳效果。
...
```{r}
grade <- sample(1:100, 10, replace=T) #代表1-100取出10個，抽出後放回
grade
...
) -> letterGrade # 有回存時要螢幕印出的訊息一定要加print，不然他就只會印出最後一要被執行的指令
letterGrade
```
...
nowtime <- ymd_hms(glue::glue("{today()} 18:00:00"),tz="Asia/Taipei")
#以上兩行都是把nowtime設為今天下午六點
typeof(nowtime)
#看typeof(nowtime)，發現他是double，代表他可以用cut
library(lubridate)
library(glue)
...
cutpoints <-
 ...
 ymd_hms(cutpoints,tz="Asia/Taipei") -> cutpoints #把cutpoints由字串轉成時間
...
practice:上面的流程其實也可以用if… else if… else來做而不用switch，請用if… else if… else來創造whatNeedToDo_butler_mom。
